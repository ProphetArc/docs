...
![Normal Node](../.gitbook/assets/normal_point.png)
...
![Hostage Rescue Node](../.gitbook/assets/hostage_rescue_point.png)
...
![Camp Points](../.gitbook/assets/camp_points.png)
...
![Camp Directions](../.gitbook/assets/camp_directions.png)
...
* On an As\_ map, the **Map Goal** node tells the bots where the VIP escape zone is. Make sure the escape zone symbol is visible on your HUD when you place a map goal node there. Otherwise the VIP may end up reaching the point and running away again just like you would do with Rescue nodes.
* On a Cs\_ map, the **Map Goal** node marks the position of the hostages. It is **NOT** necessary to place one Map Goal node per hostage. Unless the hostages are standing really far away from each other, one point per hostage group will perfectly do.
* On a De\_ map, the **Map Goal** node marks the bomb spots. It must be placed somewhere inside the bomb zone, i.e. the bomb icon must be blinking on your HUD when you place such a node. In contrast to Cs\_ maps, on De\_ maps it makes sense to set various goal nodes in one bomb zone. This will enable bots to choose from several spots to plant the bomb and make them less predictable.
* On Es\_ maps, the **Map Goal** node marks the escape zone for the Terrorists. You can follow the same rules as for As\_ maps.
...
![Hostage Goal Node (map cs\_assault)](../.gitbook/assets/hostage_goal_point.png)

![VIP Escape Goal Node (map as\_oilrig)](../.gitbook/assets/vip_goal_point.png)

![Bomb place Goal Node (map de\_dust2)](../.gitbook/assets/bombplace_goal_point.png)

![Terrorists escape Goal Node (map es\_trinity)](../.gitbook/assets/terrorists_escape_goal_point.png)
